...
def summarise(opts, del_no, cnames):
    if 'P' in cnames:
        logger.logger.info(
    ...
    if 'INFO' in cnames:
        logger.logger.info('{0} SNPs with imputation score < {0} were deleted'.format(del_no['INFO0']))
    if opts.dropna:
...
def parse_arg():
    ...
    parser.add_argument('--info', default=1, type=float, metavar='0-1', help='imputation score threshold')
    ...
    parser.add_argument('--rmpali', action='store_true', help='Drop palindromic SNPs')
    ...
    group.add_argument('--unique', action='store_true', help='Drop duplicated SNPs depending on --snpid parameter')
...
def split_df(df: pd.DataFrame, flag: pd.Series) -> tuple:
    '''
    将df按flag分组两组，一组是flag为true的数据，一组是flag为False的数据
    '''
